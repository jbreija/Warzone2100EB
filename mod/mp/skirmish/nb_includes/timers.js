...
_global.setTimers = function() {
	...
	setTimer("buildFactories", 250);
	setTimer("buildCyborgFactories", 350);
	setTimer("buildVTOLFactories", 450);
	setTimer("buildResLabs", 390);
	setTimer("buildPowGens", 130);
	setTimer("upgradeModule", 420);
	setTimer("checkProduction", 160);
	setTimer("checkResearch", 150);
	setTimer("adaptCycle", 100);
	setTimer("rebalanceGroups", 10000 + 20 * rnd());
	setTimer("checkAttack", 300);
	//setTimer("captureSomeOil", 150);
	if (difficulty === EASY)
