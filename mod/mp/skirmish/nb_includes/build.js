...
_global.upgradeModule = function(_struct) {
			buildModules(_struct)
}

_global.buildFactories = function() {
	if (playerPower(me) > 2500) {
		if (enumTrucks().filter(truckFree).length === 0)
			return;
		else {
			buildBasicStructure(structures.factories, IMPORTANCE.PEACETIME)
		}
	}
}

_global.buildCyborgFactories = function() {
	if (playerPower(me) > 2000) {
		if (enumTrucks().filter(truckFree).length === 0)
			return;
		else {
			buildBasicStructure(structures.templateFactories, IMPORTANCE.PEACETIME)
		}
	}
}

_global.buildVTOLFactories = function() {
	if (playerPower(me) > 4000) {
		if (enumTrucks().filter(truckFree).length === 0)
			return;
		else {
			buildBasicStructure(structures.vtolFactories, IMPORTANCE.PEACETIME)
		}
	}
}

_global.buildResLabs = function() {
	if (playerPower(me) > 3000) {
		if (enumTrucks().filter(truckFree).length === 0)
			return;
		else {
			buildBasicStructure(structures.labs, IMPORTANCE.PEACETIME)
		}
	}
}

_global.buildPowGens = function() {
	if (countStructList(structures.gens) < countStructList(structures.derricks)/4)
	{
		if (enumTrucks().filter(truckFree).length === 0)
			return;
		else {
			buildBasicStructure(structures.gens, IMPORTANCE.PEACETIME)
		}
	}
}
////////////////////////////////////////////////////////////////////////////////////////////
